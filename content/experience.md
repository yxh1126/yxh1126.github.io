# Experience

## Staff Embedded Security Engineer · Inceptio Technology

Santa Clara, CA · Sep. 2021 - Present

- Secure Boot and Secure OTA solutions tailored for diverse SoC architectures.
- Hardware Security (TPM, TEE, HSM/SHE) based solutions for Data Storage & Secure Communication.
- System bring-up including Driver & Device Tree integration, U-Boot updating and peripheral validation.
- Asymmetric-key-based identity authentication solution for AD (Autonomous Driving) hardware platform.

## Senior Firmware Engineer · Advanced Micro Devices

Santa Clara, CA · Oct. 2020 - Aug. 2021

- Endorsement Key Certificate verification for firmware based TPM (Trusted Platform Module).
- Regression Test for firmware TPM functionality on Microsoft Windows.

## Cybersecurity Software Engineer · Magna Electronics

Auburn Hills, MI · Nov. 2019 - Sep. 2020

- In-vehicle key management based on SHE (Secure Hardware Extension) specification.
- Secure Boot on an AUTOSAR-based software architecture.

## Embedded Software Engineer · Delphi Technologies

Troy, MI · Jul. 2016 - Nov. 2019

- Vehicle ECU software development, including feature design, development, code review, and testing.
